/*
	This Users Controller is the controller for signup, login, and user's profile UI.
  	routes:
		/signup 
		/login
...
/*
	****** REST API SECTION ******
	This section provides the following functionalities: 
  	1. render 'Sign Up' form
		1a. use bycrypt.js to transform user's password to salted hashes.
		1b. save user to usermodel database 
	2. render 'User profile' page
	3. render 'Login' form
		3a. use passport.js to authenticate
	4. render 'Logout' prompt
*/
	...
	User.findOne({ email: 'email' }).then(user => {
		...
		// else no existing user found so create a new user
			...
			bcrypt.genSalt(10, (err, salt) => bcrypt.hash(user.password, salt, (err, hash) => {
				...
				req.check('password', "Passwords do not match and/or min length is 6 characters").isLength({ min: 6 }).equals(req.body.passwordConfirmation);
...
// authenticate user -> if success, login user
...
module.exports.showUserLoginForm = (req, res, next) => {
